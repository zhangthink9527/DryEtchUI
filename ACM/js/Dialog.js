...
var JqueryE = {};
JqueryE.JqueryA = function(attr,ele) {
	ele = ele || this;
	ele = Jquery(ele);
	return ele.getAttribute?ele.getAttribute(attr):null;
}
JqueryE.getTopLevelWindow = function(){
	var pw = window;
	while(pw!=pw.parent){
		pw = pw.parent;
	}
...
JqueryE.hide = function(ele) {
	ele = ele || this;
	ele = Jquery(ele);
  ele.style.display = 'none';
}
JqueryE.show = function(ele) {
	ele = ele || this;
	ele = Jquery(ele);
  ele.style.display = '';
}
JqueryE.visible = function(ele) {
	ele = ele || this;
	ele = Jquery(ele);
...
Core.attachMethod = function(ele){
	...
	try{
		...
		for(var prop in JqueryE){
			ele[prop] = win.JqueryE[prop];
		}
	}catch(ex){
...
Dialog.prototype.show = function(){
	var pw = JqueryE.getTopLevelWindow();
	var doc = pw.document;
...
if ( !this.ParentWindow ){
	...
	if(this.innerHTML){
		...
		arr.push("' id='_DialogFrame_"+this.ID+"' allowTransparency='true'  width='100%' height='100%' frameborder='0' style=\"background-color: Jquerytransparent; border:none;\"></iframe>");
	}
	arr.push("        </div></td></tr>");
	...
	arr.push("           	<input id='_ButtonOK_"+this.ID+"'  type='button' style='width:70px;' value='OK'>");
	arr.push("           	<input id='_ButtonCancel_"+this.ID+"'  type='button' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" style='width:70px;margin-left:20px;' value='CANCEL'>");
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='Jquery(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var bgdiv = Jquery("_DialogBGDiv");
	if(!bgdiv){
		bgdiv = pw.document.createElement("div");
		bgdiv.id = "_DialogBGDiv";
		JqueryE.hide(bgdiv);
	 	JqueryT("body")[0].appendChild(bgdiv);
		if(isIE6){
	...
	var div = Jquery("_DialogDiv_"+this.ID);
	if(!div){
		...
		//div.setAttribute("dragStart","Dialog.dragStart");
	 	JqueryT("body")[0].appendChild(div);
	}
	/*div.onmousedown = function(evt){
		var w = JqueryE.getTopLevelWindow();
	...
	if(this.innerElementId){
		var innerElement=Jquery(this.innerElementId);
		...
		if(isIE){
			...
			Jquery("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			Jquery("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	Jquery("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		Jquery("_DialogFrame_"+this.ID).DialogInstance = this;
	Drag.init(Jquery("_draghandle_"+this.ID),Jquery("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		Jquery("_DialogDiv_"+this.ID).dialogId=this.ID;
		Jquery("_DialogDiv_"+this.ID).onDragStart = function(){Jquery("_Covering_"+this.dialogId).style.display=""}
		Jquery("_DialogDiv_"+this.ID).onDragEnd = function(){Jquery("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = Jquery("_ButtonOK_"+this.ID);
	this.CancelButton = Jquery("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		JqueryE.show(Jquery("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			Jquery("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			Jquery("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		Jquery("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
	...
	if(!this.AlertFlag){
		...
		//this.ParentWindow.JqueryD = this;
	}
	this.OKButton.focus();
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=JqueryE.getTopLevelWindow().Jquery(this.innerElementId);
		...
		if(isIE){
			...
			fragment.innerHTML=innerElement.outerHTML;
			innerElement.outerHTML="";
			JqueryT("body")[0].appendChild(fragment)
		}else{
			JqueryT("body")[0].appendChild(innerElement)
		}

	}
	if(this.WindowFlag){
		this.ParentWindow.JqueryD = null;
		this.ParentWindow.JqueryDW = null;
		...
		if(this.AlertFlag){
			JqueryE.hide(Jquery("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = JqueryE.getTopLevelWindow();JqueryE.hide(w.Jquery("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = JqueryE.getTopLevelWindow();
	Jquery("_DialogButtons_"+this.ID).JqueryT("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	Jquery("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	Jquery("_AlertBGDiv").style.display="";
	JqueryE.hide(Jquery("_ButtonOK_"+diag.ID));
	var win = Jquery("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	var arr = [];
	arr.push("<table height='100%' border='0' align='center' cellpadding='10' cellspacing='0' style='font-size:14px;'>");
	...
	Jquery("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	Jquery("_AlertBGDiv").style.dispaly="";
	var win = Jquery("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
...
PageOnLoad=function (){
	...
	if(d){
		try{
			d.ParentWindow.JqueryD = d;
			d.ParentWindow.JqueryDW = d.Window;
			var flag = false;
			if(!this.AlertFlag){
				...
				if(!flag){
					JqueryE.getTopLevelWindow().Jquery("_DialogBGDiv").style.opacity="0.4";
					JqueryE.getTopLevelWindow().Jquery("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				JqueryE.show(JqueryE.getTopLevelWindow().Jquery("_AlertBGDiv"));
			}
			if(d.ShowButtonRow&&JqueryE.visible(d.CancelButton)){
...
Dialog.onKeyDown = function(event){
	...
	if(event.keyCode==27){//ESC键
		...
		if(Dialog._Array.length>0){
			//Page.mousedown();
			//Page.click();
			var diag = Dialog.getInstance(Dialog._Array[Dialog._Array.length-1]);
...
Dialog.setPosition=function(){
	...
	{
		Jquery("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	//pw.Jquery(this.bgdivID).style.width= sw + "px";
	Jquery(this.bgdivID).style.height= sh + "px";
}
